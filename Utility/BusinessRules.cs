...
namespace Utility
{
    ...
    public static class BusinessRules
    {
        ...
        public static string RegisterError(int errorCode)
        {
            ...
            if (errorCode == -100)
            {
                errorValue = "Student cannot register for a course in which there is already an ungraded registration.";
            }
            else if (errorCode == -200)
            {
                errorValue = "Student has exceeded maximum attempts on mastery course.";
            }
            else if (errorCode == -300)
            {
                errorValue = "An error has occurred while updating the registration.";
            }
            else
            {
        ...
        /// <param name="parsedAtIndex">The index of the string to begin parsing.</param>
        /// <returns>A parsed string</returns>
        public static string ParseString(string stringToBeParsed, string parsedAtIndex)
        {
            return stringToBeParsed.Substring(0, stringToBeParsed.LastIndexOf(parsedAtIndex));
        }
    }
